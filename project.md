...
>> 配置文件位置:  /etc/gitconfig   （/  根目录）

>* 配置当前用户： git config --global [选项]
>> 配置文件位置:  ~/.gitconfig     （～ 主目录家目录）
...
2. 查看本地仓库状态
tarena@tarena:~/gitproject$ (在该目录下)
> git  status
...
> 说明: -m表示添加一些同步信息，表达同步内容
	file 省略表示提交所有文件  
	message 可以写对自己提交文件的备注
```
e.g.  将暂存区所有记录同步到仓库区（增删改均可）
git commit  -m 'add files'
```
6. 查看commit 日志记录 （取前7位）
> git log
> git log --pretty=oneline （简化显示 只显示一行）

7. 比较工作区文件和仓库文件差异（对文件进行更改之后工作区与仓库的差别）

> git diff  [file]

8. 将暂存区或者某个commit点文件恢复到工作区（认为之前仓库存储文件较工作区修改文档更好，可以进行文档恢复）
...
> git reset --hard HEAD^
> 注意 ： 一个^表示回退1个版本，依次类推。仓库区与工作区均退回，暂存区依旧干净。
2. 退回到指定的commit_id节点
...
> 说明: 将工作区未提交的修改封存，让工作区回到修改前的状态 (工作区未修改)
...
4. 合并分支（返回master，使用语句可将分支修改合并到master）
...
>> 当合并分支时两个分支修改了同一个文件，则需要手动解决冲突。（在文档中手动删除，之后再进行add，commit操作）
